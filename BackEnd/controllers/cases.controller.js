...
class CasesController {
  async createCase(req, res) {
    ...
    const savedDocs = [];
    try {
      CaseValidator.validateCaseData({
        title,
        description,
        status,
        court,
      ...
      for (const index in parties) {
        ...
        const newParty = new Party({
          lawyer,
          case: newCase,
          name: DBConfig.PARTY_NAMES[index],
          stakeholders: [],
        });
        const {
          idNumber,
        ...
        const newClient = new Stakeholder({
          type: DBConfig.STAKEHOLDER_TYPES[0],
          party: newParty._id,
          idNumber,
          firstName,
          lastName,
  ...
  async fileACase(req, res) {
    ...
    const savedDocs = [];
    try {
      CaseValidator.validateFilaACaseData({
        title,
        description,
        court,
        parties,
  ...
  async getUserCases(req, res) {
    ...
    try {
      ...
      const cases = [
        ...
        ...(await Party.find({ lawyer: userId }).populate("case")).map(
          (party) => party.case
        ),
      ];
      if (cases.length === 0) {
  ...
  async updateCase(req, res) {
    ...
    try {
      CaseValidator.validateUpdateCaseData({
        caseId,
        title,
        description,
        status,
      ...
      const updatedCase = await Case.findByIdAndUpdate(
        caseId,
        { $set: { title, description, status, court, judge } },
        { new: true }
      );
      if (updatedCase === null) {
        throw new CaseDoesNotExistError();
